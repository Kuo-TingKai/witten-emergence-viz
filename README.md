...
這是一場**兩種量子重力綱領的對撞**：你是 **全像糾纏（It from Qubit）** 陣營，對手是 **Carlo Rovelli**，操一套專屬的 **迴圈量子重力（Loop Quantum Gravity）** 牌組（和樂圈、自旋網絡節點、自旋泡沫、哈密頓約束、體積量子、面積量子化、大反彈、微分同胚不變、普朗克離散、Immirzi 參數）。Rovelli 是個**反應式 AI**：你鋪場他用自旋泡沫清場、你逼臉他立體積量子當牆、你下大單位他用哈密頓約束點掉——有來有回，直到分出勝負並結算。

- **資源叫「糾纏」**：每回合上限 +1（最多 10），呼應 It from Qubit。
- **招牌規則「面積＝糾纏」**：你場上相鄰單位每對形成一條金色連線，回合開始時本體護盾＝連線數（RT 公式）。
- **牌庫抽乾後的「熵增」疲勞傷害**：抽不到新資訊，熵照漲。
- **抽鬼牌式手牌**：滑鼠或手指滑過手牌時，指到的那張會站直、放大、比其他牌高一截，方便看清楚再決定出哪張。
...
Carlo Rovelli 的牌組（迴圈量子重力）—— 機制與你的牌一一對應，但物理意義來自 LQG：

| 卡 | 費 | 效果 | 對應物理 |
|---|---|---|---|
| 和樂圈 | 1 | 抽 2 張 | 沿閉合迴圈平行移動取得迴圈變數（LQG 命名由來） |
| 自旋網絡節點 | 2 | 入場時若已有單位 +1/+1 | 節點承載體積、連線承載面積，網越密越重 |
| 自旋泡沫 | 3 | 敵全場 2 點衰減 | 對所有時空歷史求和 = LQG 版路徑積分 |
| 哈密頓約束 | 2 | 指定目標 4 傷害 | 強制幾何依動力學演化 |
| 體積量子 | 3 | 1/6 嘲諷 | 離散的體積本徵值堵住通路 |
| 面積量子化 | 2 | 護盾 +2×己方單位數 | 面積只能取離散本徵值 √(j(j+1)) |
| 大反彈 | 2 | 從棄牌堆隨機重構 1 張回手 | LQC 以大反彈取代大霹靂奇點 |
| 微分同胚不變 | 4 | 3/3 免疫法術與衰減 | 規範不變量法術抓不到 |
| 普朗克離散 | 4 | 0/7 結構，本體受傷 −1 | 存在最小長度，時空在普朗克尺度離散 |
| Immirzi 參數 γ | 3 | 接下來 3 回合對敵本體 2 傷害，不可移除 | γ 定住面積間隙的尺度 |

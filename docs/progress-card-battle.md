...
## M10 — 修「打不動 Carl」的機制 + 機制 review + Carl Q 版粒子頭像

回報：玩家一直沒辦法讓 Carl 扣血。

**診斷（jsdom 跑 200 局統計 Carl 防禦）**：不是硬 bug —— Carl 會死 20%、平均最低 HP 7.7（打得動），
但防禦疊太兇：**結構（普朗克，本體傷害 −1）在玩家回合有 57% 在場**、嘲諷牆 39%、平均護盾 1.64。
小單位 chip 的傷害被 −1＋護盾＋嘲諷層層吃掉 → 體感「打不動」。

- **結構傷害減免只擋「軟」傷害**：`dmgHero` 改成只有 `o.soft`（法術 / 自旋泡沫 AoE / 熵）才 −1，
  **單位直接攻擊不再被結構減免**（邏輯算子照樣打進來）。`aoe2`/`bolt4` 對本體的傷害標記 `soft:true`。
  → 清掉嘲諷後單位能穩定扣 Carl 本體血。卡面文案同步改成「法術與熵 −1（擋不住單位攻擊）」(zh/en)。
  複測：Carl 死亡率 20%→**27%**、平均最低 HP 7.7→**6.7**（用心推臉的真人差距更大）。
- **防呆**：`.bt-ehand{ pointer-events:none }`，Carl 的牌背純裝飾、不會擋到點他本體攻擊的點擊。
- **機制 review 其他發現**：
  - 招牌護盾「回合開始＝糾纏連線數」會**覆蓋**（非疊加）RT/面積量子化加的護盾 → 那兩張盾主要是
    擋對手下一回合的傷害、不會留到你自己進攻。這是 teaching 規則的刻意設計，保留，但已知。
  - 嘲諷（1/6 體積量子）＋護盾屬正常機制，結構修正後已可被單位突破，保留。
  - CS / Immirzi 持續傷害不算 soft → 不被結構減免，成為對烏龜流 Carl 的穩定 chip（順帶小 buff，合理）。
- **Carl Q 版粒子頭像**（仿 `spirit.js` Witten 英靈手法）：新增 `rovelliSVG()` 用發光粒子 + 淡金連線
  勾出 Rovelli 辨識特徵——**滿頭波浪頭髮（非禿頂）、沒戴眼鏡、溫和微笑**，與 Witten 明顯區隔；
  渲染在 Carl 的 `.bh-portrait`（`.bh-rovelli`，香檳色粒子閃爍、尊重 reduced-motion）。
  玩家頭像維持 lucide icon。實測：Carl 頭像 82 粒子 + 34 連線、玩家仍 lucide。
- **驗證**：i18n zh/en 97/97、`game.css` 208/208；120 局回歸 100% 分勝負、0 JS 錯誤。
- Fallback：`dmgHero` 還原 `!o.noHappy` 判斷、移除 `soft` 標記即回舊減免；移除 `rovelliSVG()`、
  portrait 改回 `orbit` icon、刪 `.bh-rovelli` CSS 即還原頭像。

## M11 — 即時教練提示（可開關）：告訴玩家該出哪張牌

回報：一直贏不了 Carl，希望有即時輔助提示說明何時出哪張牌（手牌發光 + 提示文字），可開關。

- **`advise()` 建議引擎**（玩家視角，目標＝打贏 Carl）：依場面回傳單一「下一步」建議——
  斬殺偵測 → 嘲諷牆擋路提示先清/轟掉 → 解大單位 → 鋪場（先大單位）→ 補抽/持續傷害/清場/補盾/重構 →
  打臉 → 結束回合。每個建議帶一句教學理由（i18n `game.advice`，zh+en）。
- **UI**：
  - 建議「出某張牌」→ 該手牌站直上抬、金光脈動（`.bt-card.advise`），**上方浮出提示氣泡 `.bc-tip`**
    （例：「先用這張轟掉嘲諷牆，才打得到 Carl 本體」）；若是指定法術，連帶把建議的目標單位也發光。
  - 建議「攻擊 / 結束回合」→ 底部教練橫幅 `.bt-coach` 顯示理由，並讓可攻擊單位發光。
  - 中央控制列加「💡」開關鈕（`.bt-advisor`，亮起＝開），**預設開啟**（新手需要），
    狀態存 `localStorage["witten-battle-advisor"]`。
- 教練建議特別點出關鍵打法：「結構擋不住單位攻擊」「先清嘲諷牆」「CS/Immirzi 對烏龜流有效」——
  直接教玩家怎麼突破 Carl 的防禦。
- **驗證**：jsdom —— 開局無可出牌時橫幅顯示「結束回合」建議；有可出牌時該牌發光 + 氣泡文字正確；
  💡 可開關（關閉後 `.advise` 消失）；advise() 每次 render 重算、100 局回歸 100% 分勝負、0 JS 錯誤。
  i18n zh/en **116/116 對齊**、`game.css` 222/222 平衡。
- Fallback：移除 `advise`/`advisor`/`curAdvice`、`.bt-advisor`/`.bt-coach` 與相關 CSS、i18n `advice`
  區段即還原 M10。

## M12 — 出牌用光時明確提示玩家結束回合

回報：一回合中能出的牌用光後，玩家不知道該結束回合了（M7 的自動結束 + 鈕脈動太隱晦）。

- **明確提示橫幅 `.bt-endhint`**：render 每次檢查「玩家回合、非 busy/結束/指定中、且手牌已無任何可出的牌」，
  浮出一條金色提示（含向下小箭頭、指向中央「結束回合」鈕、脈動）：
  - 還有單位可攻擊 → 「沒有可以出的牌了 —— 用單位攻擊，或點『結束回合』」（`endHintAttack`）
  - 也沒得攻擊 → 「這回合沒有可以出的牌了 —— 結束回合（即將自動交給 Rovelli）」（`endHintNone`，
    搭配 M7 的 750ms 自動結束）。
- **不依賴教練開關**：這是基本可用性提示，`advisor` 關掉也照常顯示（與 M11 教練橫幅互補、位置不同）。
- i18n `game.endHintNone/endHintAttack`（zh+en）。
- **驗證**：jsdom —— turn1 還有可出牌時不顯示；出光牌且有攻擊單位時顯示 `endHintAttack`（文字正確）；
  100 局回歸 100% 分勝負、0 JS 錯誤。i18n zh/en **118/118 對齊**、`game.css` 230/230 平衡。
- Fallback：移除 `.bt-endhint` 的 render 邏輯/DOM/CSS 與 i18n 兩鍵即還原 M11。

## M13 — 攻擊類教練提示改成「可操作的兩步指引」

回報：M11 的「Carl 有嘲諷牆擋路 → 先用單位把它清掉」太抽象，連開發者都看不懂到底要點哪張牌 / 做什麼操作。
根因：攻擊類建議（attackFace/attackTaunt）沒有發光任何具體單位、文字也只是抽象描述。

- **advise() 攻擊建議補上具體目標**：回傳 `attackerUnit`（攻擊力最高、建議先點的己方單位）與
  `targetUnit`（要打的嘲諷牆）。
- **兩步發光指引**：
  - 第一步（還沒選單位）→ 該點的**己方攻擊單位發光**（`.bt-unit.advise`）。
  - 第二步（已選單位 pending attack）→ 該打的**敵方目標發光**：嘲諷牆 / 大單位（`.bt-unit.advise`）
    或 Carl 頭像（新增 `.bt-hero.advise`）。
  - 指定法術（clearTaunt/removeThreat）同樣在出牌後讓目標單位發光。
- **文字改成明確操作步驟**（i18n，zh+en）：
  - attackTaunt：「Carl 的嘲諷牆擋路：**點你發光的單位，再點 Carl 發光的嘲諷牆**把它清掉」
  - attackFace：「**點你發光的單位，再點 Carl 頭像**攻擊他本體（結構擋不住單位！）」
  - lethal：「可以斬殺！**點你發光的單位，再點 Carl 頭像**收掉這局」
  - clearTaunt/removeThreat：「**打出這張法術，再點 Carl 發光的**嘲諷牆/大單位…」
- **驗證**：jsdom 重現嘲諷情境 —— 第一步建議單位(tqft)發光 + 橫幅顯示兩步操作文字；點下去後第二步
  目標(體積量子嘲諷牆)發光。100 局回歸 0 JS 錯誤；i18n zh/en 118/118、`game.css` 230/230。
- Fallback：advise() 攻擊建議拿掉 `attackerUnit/targetUnit`、renderRow/renderHero 的 `.advise`
  指引與文字還原即回 M12。
